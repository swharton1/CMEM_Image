...
#This file will use the updated smile object to get a realistic pointing direction. IT WILL USE THE UPDATED PREDEFINED ORBITS IN /DATA/SMILE/SHARED/SXI_SKYBGD/, INSTEAD OF YASIR'S FILE. 
import CMEM_Image
import numpy as np
import os
import pickle
...
orbit_num=10 #From 1-12. 
#stime=(2025,10,1)
#etime=(2025,10,3,3)
#stime=(2026,4,2)
#etime=(2026,4,4,3)
#orbit = CMEM_Image.load_ephemeris_vlocal.orbit(stime=stime, etime=etime, calc_gsm=True)
orbit = CMEM_Image.load_orbit_data.orbit(orbit_num=orbit_num)
orbit_data = orbit.data 

#DO NOT NEED TO APPLY SUN AND ALTITUDE CONSTRAINTS MYSELF. 
...
orbit_folder_name = "orbit_{:0>2}".format(orbit_num) 
...
for i in range(len(rv)):
    ...
    if i%6 == 0:
        ...
        #Put constraint conditions here to only calculate when the radial and solar conditions are met. 
        ...
        output_dict['x_gse'].append(x_gse[i])
        output_dict['y_gse'].append(y_gse[i])
        output_dict['z_gse'].append(z_gse[i]) 
